...
class _ListRefreshState extends State<ListRefresh> {
  ...
  Future<List> _fetchDataFromNet() async {
    if (widget.requestAPI is Function) {  // requestAPI 是个map
      final listObj = await widget.requestAPI({'pageIndex': _currentPageIndex});  // page++ 在调用页执行
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context,index){
          ...
          ///焦点图
          if (index == 0 && index != dynamicList.length) {
          ...
          if (index == dynamicList.length) {
            ...
            if (widget.renderItem is Function) {
              return widget.renderItem(index,dynamicList[index]);
            }
